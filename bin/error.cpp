...
char *outfilename3= (char *)malloc(size);	
char *fm1 = (char *)malloc(size);
	char *fm = (char *)malloc(size);
	
	fm1 = get_file_name("../data/error/",argv[2] , argv[3]);
	outfilename = get_file_name(fm1, ".", "txt");
outfilename3=get_file_name(fm1,"_mpfr",".txt");	
fwp_out = fopen(outfilename, "wb+");
fwp=fopen(outfilename3,"wb+");
	
	fm= get_file_name("../data/",argv[4] , argv[3]);
	fm= get_file_name(fm, "/", argv[2]);
outfilename2=get_file_name(fm,".","txt");
	outfilename1 = get_file_name("../data/oracle/", argv[2], ".txt");
	fp = fopen(outfilename2, "rt");
	fp1 = fopen(outfilename1, "rb");
printf("%s %s %s %s\n",outfilename2,outfilename1,outfilename3,outfilename);
	int flag;
	//while ((!feof(fp))) {
int num;
num=atoi(argv[1])*4096;
for(int i=0;i<num;i++){
//printf("2");		
fscanf(fp, "%16lX", &test.parts.sw);
		mpfr_myr(&mpfr_result, PRECISION, fp1);
		if (!feof(fp)) {
//printf("1");
			a = test.value;
			flag=std::isinf(test.value);          
 			if(flag!=0)                     
 				mpfr_set_inf(result,1);         
 			else                            
 			{                               
                Rerror(a, mpfr_result, result);
 			}
			
			aa = mpfr_get_d(result,MPFR_RNDD);
			test1.value=aa;
mpfr_myw(&result, PRECISION, fwp);            
